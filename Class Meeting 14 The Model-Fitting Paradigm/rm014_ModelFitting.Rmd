...
#I creates new variable on the plot
...
#did not finish chunk below, had to make it normal text in order to knit 
years1 = FILL_THIS_IN
...
```{r}
ggplot(gapminder_Zimbabwe, aes(year,lifeExp)) + geom_point()+geom_smooth(method = "lm", )
```
Now we will try to fit a second degree polynomial and see what would that look like.
```{r}
ggplot(gapminder_Zimbabwe, aes(year, lifeExp)) + geom_point()+geom_smooth(method= "lm", formula= y~ poly (I(year-1952), degree= 2))
```
