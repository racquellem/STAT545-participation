---
title: "RM012: Working with factors in R"
...
#Only four categories, because none of the coutnries had lfie expectancy less than 23. So by default R doesn't want to add it. HOWEVER, we do want it in our graph.

# Labels are not in order, did it alphabetically (low to high is what we wanted)
...
h_countries= c("Canada", "Ireland", "Madagascar", "Egypt", "Haiti")
gap = gapminder %>% 
...
# can use maximum too buy just replacing min with max, or if you dont care about finding min and max then just delete that part altogether.
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_relevel(continent, "Oceania", "Americas", "Europe", "Asia", "Africa"))) +
  coord_flip()+
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
